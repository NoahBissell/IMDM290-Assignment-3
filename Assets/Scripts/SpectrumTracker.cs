...
public class SpectrumTracker : MonoBehaviour
{
    AudioSource source;
    ...
    public float GetAmpInRange(FrequencyRange range)
    {
    ...
    public float GetAmpInRangeSmooth(FrequencyRange range, float current, ref float velocity, float smoothTime)
    {
        return Mathf.SmoothDamp(current, GetAmpInRange(range), ref velocity, smoothTime);
    }
}